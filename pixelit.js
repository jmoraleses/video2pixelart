...
class pixelit {
  constructor(config = {}) {
    ...
    this.scale =
      config.scale && config.scale > 0 && config.scale <= 200
        ? config.scale * 0.01
        : 8 * 0.01;
   ...
   */
  setScale(scale) {
    this.scale = scale > 0 && scale <= 200 ? scale * 0.01 : 8 * 0.01;
    return this;
  }
  ...
  pixelate() {
    ...
    tempCanvas.width = Math.max(natW, scaledW);
    tempCanvas.height = Math.max(natH, scaledH);
